import FilterBuilder from "../../builder/filter.builder";
  ...
  describe('root', () => {
    ...
    it('Filter Builder with Element filters', () => {
      expect(FilterBuilder({
        ...
        value: true,
        operation: "$exists"
      })).toStrictEqual({"_id": {"$exists": true}});
      expect(FilterBuilder({
        field: "_id",
        value: false,
        operation: "$exists"
      })).toStrictEqual({"_id": {"$exists": false}});
      expect(FilterBuilder({
        field: "qty",
        value: false,
        operation: "$exists"
      })).toStrictEqual({"qty": {"$exists": false}});
      expect(FilterBuilder({
        field: "qty",
        value: "double",
        operation: "$type"
      })).toStrictEqual({"qty": {"$type": "double"}});
      expect(FilterBuilder({
        field: "qty",
        value: "number",
        operation: "$type"
      })).toStrictEqual({"qty": {"$type": "number"}});
      expect(FilterBuilder({
        field: "roles",
        value: "array",
        operation: "$type"
      })).toStrictEqual({"roles": {"$type": "array"}});
      expect(FilterBuilder({
        field: "createdAt",
        value: "timestamp",
        operation: "$type"
      })).toStrictEqual({"createdAt": {"$type": "timestamp"}});
    });
    ...
    it('Filter Builder with evaluation filters', () => {
      expect(FilterBuilder({
        ...
        value: "^hello.*ld$",
        operation: "$regex"
      })).toStrictEqual({"name": {"$regex": '^hello.*ld$'}});
      expect(FilterBuilder({
        value: {required: ["item", "qty", "instock"]},
        operation: "$jsonSchema"
      })).toStrictEqual({$jsonSchema: {required: ["item", "qty", "instock"]}});

      expect(FilterBuilder({value: {$search: "coffee"}, operation: "$text"}))
        .toStrictEqual({$text: {$search: "coffee"}});

      const $whereFunction = () => function () {
        // @ts-ignore
        return this.name == 'Mee Doo'
      };

      expect(FilterBuilder({value: $whereFunction, operation: "$where"}))
        .toStrictEqual({"$where": $whereFunction});
    });
    ...
    it('Array filter without array value', () => {
      ...
      expect(() => FilterBuilder({
        ...
        value: "Fail Test",
        operation: "$in"
      })).toThrowError(`The value for _id must be an array`)
      expect(() => FilterBuilder({field: "_id", value: "Fail Test", operation: "$nin"})).toThrowError(Error)
      expect(() => FilterBuilder({
        field: "qty",
        value: "Fail Test",
        operation: "$in"
      })).toThrowError(`The value for qty must be an array`)
    })

    it('Logical conditions', () => {
      expect(FilterBuilder({
        value: [
          FilterBuilder({field: 'name', value: 'a', operation: '$eq'}),
          FilterBuilder({field: 'name', value: 'b', operation: '$ne'}),
        ], operation: "$and"
      }))
        .toStrictEqual({$and: [{name: {$eq: 'a'}}, {name: {$ne: 'b'}}]})
    })
  });
});
